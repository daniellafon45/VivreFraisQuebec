...
async function getWikiImage(cityName) {
    ...
    if (validImages.length === 0) {
        console.log(`[fail] no valid images found for ${cityName} on fr.wiki`);
        return null;
    }
    ...
    if (!selectedImageTitle) {
        console.log(`[fail] selectedImageTitle is null for ${cityName}`);
        return null;
    }
    ...
    if (!imgData || !imgData.query || !imgData.query.pages) {
        console.log(`[fail] bad imgData for ${cityName}`);
        return null;
    }
    ...
    if (imgPageId === '-1' || !imgPages[imgPageId].imageinfo || imgPages[imgPageId].imageinfo.length === 0) {
        console.log(`[fail] missing imageinfo for ${cityName}`);
        return null;
    }
